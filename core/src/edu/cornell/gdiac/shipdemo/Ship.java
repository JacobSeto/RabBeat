...
public class Ship {
	...
	/**The number of frames until you can teleport again*/
	private static final int TELEPORT_RATE = 120;
	
    // Modify this as part of the lab
	/** Amount to scale for Default*/
	private static final float DEFAULT_SCALE = 1.0f;
    /** Amount to scale the first ship size */
    private static final float FIRST_SHIP_SCALE = 2f;

		/** Amount to scale the second ship size */
	private static final float SECOND_SHIP_SCALE = 3.0f;
    ...
    /** Countdown to limit reteleport rate*/
	...
	/**Returns the scale of the ship based on color
	 */
	public float getScale(){
		if(this.getColor().equals(new Color(1.0f, 0.25f, 0.25f, 1.0f))){
			return FIRST_SHIP_SCALE;
		}
		else{
			return SECOND_SHIP_SCALE;
		}
	}
	...
	/**
	 *  Teleport a given ship to the mouse position on screen.  Resets the countdown
	 * @param  canvasHeight height of the canvas
	 */
	public void teleportShip(int canvasHeight){
		this.pos = new Vector2(Gdx.input.getX(), canvasHeight - Gdx.input.getY());
		this.teleportCount = TELEPORT_RATE;
    ...
    public void drawShip(GameCanvas canvas) {
	    ...
	    canvas.draw(shipSprite, stint, ox, oy, sx, sy, rotate, getScale(), getScale());
	    // Then draw the ship
	    canvas.draw(shipSprite, tint, ox, oy, pos.x, pos.y, rotate, getScale(), getScale());
    }
