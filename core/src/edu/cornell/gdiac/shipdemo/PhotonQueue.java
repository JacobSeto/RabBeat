...
public class PhotonQueue {
	...
	/**the photon acceleration constant for blue ship photons*/
	private static final float BLUE_PHOTON_ACCELERATION = 1.0275f;

	/**the mass of the photon, used for collisions*/
	public static final float PHOTON_MASS = 20f;

	/**default scale of a photon*/

	public static final float PHOTON_SCALE = 1.25f;
	...
	public class Photon {
		...
		/**the type of photon a ship fires.  True for blue ship, false for red ship*/
		public boolean type;
		 ...
		 * @param t the photon type based on the ship that fired it
		 */
		public void allocate(float x, float y, float vx, float vy, boolean t) {
			this.x  = x;  this.y  = y;
			this.vx = vx; this.vy = vy;
			this.age = 0; this.type = t;
			this.scale = PHOTON_SCALE;
		}
...
//             }
	 ...
	 * @param type The type of photon the ship fires
	 */
	public void addPhoton(Vector2 position, Vector2 velocity, float angle, boolean type) {
		float fire_x;
		float fire_y;
		if(type){
			//blue ship projectile

			// Determine direction and velocity of the photon.
			fire_x = velocity.x + (float)Math.cos(Math.toRadians(angle))  * PHOTON_VELOCITY *.3f;
			fire_y = velocity.y + (float)-Math.sin(Math.toRadians(angle)) * PHOTON_VELOCITY * .3f;
		}
		else{
			//red ship projectile

			// Determine direction and velocity of the photon.
			fire_x = velocity.x + (float)Math.cos(Math.toRadians(angle))  * PHOTON_VELOCITY * 1.5f;
			fire_y = velocity.y + (float)-Math.sin(Math.toRadians(angle)) * PHOTON_VELOCITY * 1.5f;
		}
	...
	public void move(Rectangle bounds) {
		...
		if(queue[idx].type){
			//blue ship bullets accelerate
			queue[idx].vx *= BLUE_PHOTON_ACCELERATION;
			queue[idx].vy *= BLUE_PHOTON_ACCELERATION;
	...
	public void draw(GameCanvas canvas) {
		...
		float scale;
		if(queue[idx].type){
			//blue photons medium size
			scale = queue[idx].scale;
		}
		else{
			//red photons small but grow in size
			scale = queue[idx].scale +  .02f;
			System.out.println(scale);
			queue[idx].scale = scale;
		}
		...
		if(queue[idx].type){
			//blue ship photon
			baseColor = new Color(0,0,.5f,1);
		}
		else{
			//red ship photon
			baseColor = new Color(.5f, 0 , 0, 1);
		}
		baseColor.g = (float)queue[idx].age  / (float)MAX_AGE;
            canvas.draw(texture,baseColor,ox,oy,queue[idx].x,queue[idx].y,0,scale,scale);
        }
	}
}