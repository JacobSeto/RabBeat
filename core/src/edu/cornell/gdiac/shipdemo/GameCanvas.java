...
public class GameCanvas {
	...
	public void draw(TextureRegion region, Color tint, float ox, float oy, 
		...
		//clipping draw
		// Horizontal wrap
		boolean leftEdge = x < region.getRegionWidth()/2;
		boolean rightEdge = x > width - region.getRegionWidth()/2;
		boolean topEdge = y > height - region.getRegionHeight()/2;
		boolean bottomEdge = y < region.getRegionHeight()/2;
		if (leftEdge) {
			computeTransform(ox,oy,x+width,y,angle,sx,sy);
			spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
			if(topEdge){
				computeTransform(ox,oy,x+width,y-height,angle,sx,sy);
				spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
			}
			if(bottomEdge){
				computeTransform(ox,oy,x+width,y+height,angle,sx,sy);
		...
		// Vertical wrap
		if (topEdge) {
			computeTransform(ox,oy,x,y-height,angle,sx,sy);
			spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
		} if (bottomEdge) {
			computeTransform(ox,oy,x,y+height,angle,sx,sy);
			spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
    ...
    public void drawOverlay(Texture image, boolean fill) {
		...
		drawOverlay(image,Color.TEAL,fill);
    }
